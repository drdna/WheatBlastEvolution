...
def plot_back2back(tree_files,fig_name,**kwargs):
    ...
    yax_border = kwargs.get('yax_border', 2) # should be relative to ySpan of trees
    tip_font_size = kwargs.get('tip_font_size', 12)
    tangle_line_width = kwargs.get('tangle_line_width',12)
    ...
    for t,tr in enumerate(tree_names): ## iterate over trees
        ...
        if tr == 1:
            x_attr=lambda k: cumulative_displace + cur_tree.treeHeight-k.height  
        else:  
            x_attr=lambda k: k.height+cumulative_displace
        #x_attr=lambda k: (k.height*(max_height/cur_tree.treeHeight))+cumulative_displace
        ...
        # For tip naming
        text_func = lambda k: k.name.replace('_','x')
        target_func = lambda k: k.is_leaf()
        if tr == 1:
            position_func = lambda k: (cumulative_displace-0.06, k.y-0.35)
        else:
            position_func = lambda k: (k.height+cumulative_displace-0.06, k.y-0.35)
        ...
        for k in cur_tree.Objects: ## iterate over branches
            if isinstance(k,bt.leaf): ## if leaf...
                ...
                if tr == 0:    
                    next_x,next_y=tip_positions[tree_names[t+1]][k.name] ## fetch coordinates of same tip in next tree
                    #next_x+=cumulative_displace+cur_tree.treeHeight+displaceAmount ## adjust x coordinate by current displacement and future displacement
                    next_x+=cumulative_displace ## adjust x coordinate by current displacement and future displacement
                    nextIncrement=cumulative_displace+cur_tree.treeHeight
                    #ax.plot([x_attr(k),nextIncrement+0.05*displaceAmount,nextIncrement+0.95*displaceAmount,next_x],[y,y,next_y,next_y],lw=tangle_line_width,ls='-',color=cmap(frac_pos),zorder=0) ## connect current tip with same tip in the next tree
                    ax.plot([x_attr(k),nextIncrement+0.95*displaceAmount],[y,next_y],lw=tangle_line_width,ls='-',color=cmap(frac_pos),zorder=0) ## connect current tip with same tip in the next tree
...
if  __name__ == '__main__':
    ...
    with open(strain_map) as file:
      for line in file:
        ...
        strain_dict[strain_list[1]] = strain_list[0]
    ...
    segments = 9
    path = sys.argv[2]
    ML_tree_files = [path + "MCCT" + str(i) + ".tre" for i in range(1,segments)]
    print(ML_tree_files)
