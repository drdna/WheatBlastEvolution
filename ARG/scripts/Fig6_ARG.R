...
strains <- subset(strains, (Geno %in% c("PoL1-1","PoT1","PoL1-2", "PoL1-6","PoL1-12","PoL1-13", "PoT1", "PoT3", "PoT4", "PoT20", "PoT25", "PoT32"))) #,
...
#     width = 13*ppi, height= 7.8*ppi, res = ppi) 
#pdf(file=paste("~/Fig3A",".pdf", sep = ''), width = 14, height= 8.5) 
...
df3 <- rbind(dfE, dfLu, dfSt, dfU1, dfX, df2)

df3$order <- factor(df3$Geno, levels=c('PoE','PoLu','PoSt','PoU1', 'PoX',
                                       'PoL1-1', 'PoL1-2', 'PoL1-6', 'PoL1-12', 
                                       'PoL1-13', 'PoT1', 'PoT3', 'PoT4', 'PoT20', 'PoT25', 'PoT32'))


print(
  ggplot(df3) +   #subset(df, Recipient == "ATCC64557") 
    geom_point(mapping = aes(x=variable, y=0, color=donor, size = 6), size = 6)+  # , alpha = 0.1, show.legend = F
    scale_color_manual(name="Donor", values=colors)+
    theme_prism() +
    scale_x_continuous(guide="prism_minor", breaks = seq(0, 8, 1), minor_breaks = seq(0,8,0.1)) +
    coord_cartesian(ylim = c(-0.5,0.5)) + 
    labs(title = " ", y = " ", x= "\nChromosome position (Mb)",  colour = "Donor")+
    facet_grid(order ~ Chr, scales = "free", space = "free", switch ="y")+   #cols = vars(Chr)
    
    theme( 
           plot.margin=unit(c(0, 0.8, 0, 0), units="line"), # t, r, b, l Dimensions of each margin 
