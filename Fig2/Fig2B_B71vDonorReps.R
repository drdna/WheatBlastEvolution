...
colors <- c(U1="#D55E00", O="#0072B2", S= "#0072B2", E1="#009E73", S="#F0E442", P="#0072B2", Lu= "#999999", U3= "#AA4499")
...
for (f in 1:7) {
  ...
  # only plot comparisons for Lolium and Triticum strains
  df2 <- df %>% rownames_to_column('Strains') %>% filter(., Strains %in% c("U232", "Guy11", "B51", "Bm88324", "U169", "Br35", "Pr8202", "FPH-2015-44")) #filter(., clade %in% c("U1", "O", "S", "U3", "Lu", "E1"))
  end <- ncol(df2)
...
p<-grid.arrange(grobs = myPlots, ncol = 1, nrow = 7, heights = c(1,1,1,1,1,1,1.2), 
                top = textGrob("Haplotype Divergence: B71 versus Representative Donors", gp = gpar(fontface = "bold", cex = 1.2)),
...
pdf("B71vRepresentativeDonors.pdf", 8.5, 11)
print(p)
dev.off()
